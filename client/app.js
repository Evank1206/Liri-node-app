...
$(document).ready(function () {
    ...
    $("#btn").on("click", (e) => {
        ...
        axios.get(`http://localhost:5000/api/${inputValue}`).then(function (response) {
            ...
            // if (response.data[i].hasOwnProperty("artist")) {
            ...
            for (let i = 0; i < response.data.length; i++) {
                ...
                if (response.data[i] == response.data.length) {
                    ...
                    $(".bandName").append("<b><a href='#'>" + " Band Name: " + response.data[i].lineup + "</a></b>");

                    var concertDate = $("<span class='ui ShowDate'/>");
                    // concertDate.text("Show Date: ");
                    $(".ShowTime").append(concertDate);
                    $(".ShowDate").append("<b><a href='#'>" + "Show Date: " + response.data[i].datetime + "</a></b>");

                    var palaza = $("<p class='plaza'/>");
                    // palaza.text("Palace:  ");
                    $(".palace").append(palaza);
                    $(".plaza").append("<b><a href='#'>" + "Palace: " + response.data[i].venue.name + "</a></b>");

                    var city = $("<p class='ShowCity'/>");
                    // city.text("City:  ");
                    $(".city").append(city);
                    $(".ShowCity").append("<b><a href='#'>" + "City: " + response.data[i].venue.city + "</a></>");

                    var country = $("<p class='ShowCountry'/>");
                    // country.text("Country:  ");
                    $(".country").append(country);
                    $(".ShowCountry").append("<b><a href='#'>" + "Country: " + response.data[i].venue.country + "</a></b>");
                    ...
                    console.log("reached length of the array");
